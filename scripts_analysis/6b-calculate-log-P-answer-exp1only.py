...
# filter out Experiment 2 participants
imported_participants = imported_participants[imported_participants['learning_condition'] == "Experiment 1 (no preview)"]

# remove participants from sample that are not in imported_participants
sample = {k: v for k, v in sample.items() if k in imported_participants['participant_id'].values}
# %%
...
# export df to csv
df.to_csv(os.path.join(current_dir, '../outputs/exp1_beta_with_plus_minus_2LL_errors.csv'))
...
save_path = os.path.join(current_dir, '../outputs/exp1_trials_with_max_beta.pickle')
