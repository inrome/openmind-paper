...
max_beta_values = pd.DataFrame(columns = ['participant_id', 'fsm_type', 'task', 'feedback_condition', 'trial_type', 'max_beta'])
for participant_id in trials['participant_id'].unique():
    ...
    feedback_condition = trials_participant['feedback_condition'].unique()[0]

    # append to dataframe
    for task in trials['test_condition'].unique():
        for trial_type in ['visible', 'hidden', 'hidden_an']:
            new_row = {'participant_id': participant_id,'fsm_type':fsm_type, 'task': task, 'feedback_condition': feedback_condition, 'trial_type': trial_type, 'max_beta': max_beta[task][trial_type]}
...
wide_format_df = max_beta_values.pivot_table(index=['participant_id', 'fsm_type', 'feedback_condition'],
...
wide_format_df.rename(columns={'participant_id_': 'participant_id', 'fsm_type_':'fsm_type', 'feedback_condition_':'feedback_condition'}, inplace=True)
