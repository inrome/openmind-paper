...
actual_accuracy = trials_PEC.groupby(['participant_id', 'fsm_type', 
                                          'test_condition'])['response_correct'].mean().reset_index()
mental_model_accuracy = trials_PEC.groupby(['participant_id', 'fsm_type', 
                                          'test_condition'])['response_correct_mm'].mean().reset_index()

# make sure that the order of the rows is the same for both dataframes
actual_accuracy = actual_accuracy.sort_values(by=['participant_id', 'fsm_type', 'test_condition'])
mental_model_accuracy = mental_model_accuracy.sort_values(by=['participant_id',  'fsm_type', 'test_condition'])
