# Log P(answer) gives us a measure of how well the model with a given beta-value predicts the participants' answers
...
trials_test = pd.read_csv(os.path.join(current_dir, '../outputs/trials_test_with_PEC_mm_probs_Ri_withAccuracy.csv'))

# define function to compute log_p_answer
...
# a function to generate log_p_answers for a range of beta values
def generate_log_p_answers(trials_test, task, beta_max, beta_step):
    beta_values = np.arange(0, beta_max, beta_step)

# OLD CODE #####################################################################
def compute_log_p_answer_for_trials(trials_test, task, beta):
