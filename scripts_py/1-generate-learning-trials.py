...
for participant_id in participant_ids:
    ...
    ts_learning_1 = ts_learning_raw[['participant_id', 'fsm_number', 'test_condition', 
                                     'sender', 'timestamp', 'learning_counter','state_current', 
                                     'response_current', 'state_next', 'task', 'learning_condition']].copy()
    ...
    # filter out trials with sender in ["control_screen_task"]
    ts_learning_1 = ts_learning_1.loc[~ts_learning_1['sender'].isin(["control_screen_task"])]

    # remove duplicates (use the first attempt based on timestamp)
    if len(ts_learning_1['learning_counter']) > 49: # 50 is the number of learning trials (45) + up to 5 test preview questions
    ...
    ts_learning_2 = ts_learning_raw[['participant_id', 'learning_counter', 'sender', 'response', 'duration']].copy()
    ts_learning_2 = ts_learning_2.loc[ts_learning_2['response'].notna() & 
                                      ...
                                      ts_learning_2['sender'].isin(['response_1_screen', 'response_2_screen', 'response_1_screen_time', 
                                                                    'response_2_screen_time', 'response_further_screen', 'response_further_screen_time'])]
    ...
    # remove participants with less than 40 learning trials
    if len(ts_learning_1) < 40: 
